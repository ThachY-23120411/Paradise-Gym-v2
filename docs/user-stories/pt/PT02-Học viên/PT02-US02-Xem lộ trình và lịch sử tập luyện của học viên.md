...
## Trigger
- PT bấm chọn một gói tập cụ thể từ:
  * **Màn hình phụ Danh sách gói của học viên** (mở từ Tab 1 `Học viên phụ trách`).
  * **Thẻ gói tập** trên Tab 2 `Gói đang phụ trách`.
- Màn hình liên quan: Mobile App PT — Màn hình `Chi tiết lộ trình tập luyện của học viên` (`#clientDetailSubscreen`).

## Main Flow

1. PT bấm chọn một gói tập (từ màn hình phụ danh sách gói của học viên ở Tab 1 hoặc trực tiếp từ thẻ gói ở Tab 2).
2. Hệ thống nạp và hiển thị thông tin tổng quan lộ trình tập luyện của học viên theo gói tập đó:
...
5. Khi PT bấm nút quay lại `[←]`:
   - Nếu trước đó mở từ Màn hình phụ danh sách gói của học viên: Quay về màn hình phụ danh sách gói của học viên đó.
   - Nếu trước đó mở trực tiếp từ Tab 2: Quay về danh sách gói tại Tab 2.
6. **Quy tắc nghiệp vụ:**
...
| :--- | :--- | :--- | :--- | :--- | :--- |
| **Nút quay lại (Back Button `[←]`)** | `Action Button (Back Icon)` | `USER-INPUT` | required | Không | Icon mũi tên quay lại góc trên bên trái màn hình; chạm để đóng màn hình chi tiết và quay về đúng màn hình nguồn trước đó (Màn hình phụ danh sách gói nếu mở từ Tab 1, hoặc Tab 2 `Gói đang phụ trách` nếu mở từ Tab 2) |
...
| **Buổi đã đặt cạnh tiến độ** | `Readonly metric` | `READONLY` | required | Không | booked API; remaining là khả dụng đã loại booked |
...
| **Thẻ lịch sử buổi tập đã hoàn thành (Session History Item)** | `Timeline Session Card` | `READONLY` | conditional | `CONDITIONAL`: **Hiện khi** học viên đã có ít nhất 1 buổi tập đạt xác nhận hoàn thành (`DONE`); **Ẩn khi** học viên là người mới chưa có buổi tập nào hoàn thành | Thẻ lịch sử theo dòng thời gian (timeline), hiển thị: Thứ tự buổi tập (`Buổi 1`, `Buổi 2`...), Ngày tập (`DD/MM/YYYY`), Khung giờ (`HH:mm - HH:mm`), Badge trạng thái `Hoàn thành` (xanh lá), và Khối nội dung Đánh giá của PT đã ghi nhận sau buổi |
...
- Dữ liệu từ API theo member/registration được phân công hiện hành; end_date null hiển thị Không giới hạn, không tạo ngày hết hạn giả.
- Thanh tiến độ dùng used/total; số khả dụng lấy remaining, số đã đặt lấy booked. Tổng buổi = used + booked + remaining; không trừ buổi giữ lịch lần nữa khi xác nhận kép.
- Mỗi buổi hiển thị giờ thực tế, số thứ tự và ghi chú API. Lịch sử đã hoàn thành chỉ đọc; không thêm quyền sửa kết quả hay hủy lịch.
...
```mermaid
flowchart TB
  subgraph B["Boundary - Mobile PT / PT02-US02"]
    subgraph L0["Swimlane - PT"]
      I(("Initial"))
      A["Chọn học viên trong phân công"]
      D2{"Thao tác tiếp?"}
    ...
    subgraph L1["Swimlane - SYS"]
      M(("Merge - Tải dữ liệu"))
      S["Kiểm tra scope và tải hợp đồng/lịch sử"]
      D{"Kết quả tải?"}
      E["Báo lỗi hoặc mất quyền"]
      FE((("Final - Không thể tải")))
      EMP["Hiển thị trạng thái rỗng"]
      V["Hiển thị giờ thực tế, tiến độ và ghi chú"]
