...
| :--- | :--- | :--- | :--- | :--- | :--- |
| **Tên gói tập** | `Typography / Heading` | `PREFILL` + `READONLY` | required | Không | Tên gói tập đã chọn mua ở màn hình trước (ví dụ: `Gói PT 20 buổi`, `Combo Gym 3 tháng + PT 10 buổi`) |
| **Số tiền cần thanh toán** | `Badge / Price tag` | `PREFILL` + `READONLY` | required | Không | Giá tiền chính xác 100% của gói tập (ví dụ: `5.000.000 đ`) |
| **Phương thức thanh toán** | `Badge / Text label` | `READONLY` | required | Không | Cố định: `Chuyển khoản Ngân hàng (VietQR)` |
| **Mã QR chuyển khoản (VietQR Image)** | `QR Code Image / Graphic` | `READONLY` | required | `DYNAMIC`: Sinh theo đơn đăng ký | Ảnh mã VietQR động chứa đầy đủ thông tin: STK, Số tiền 100% và Nội dung chuyển khoản duy nhất |
| **Tên ngân hàng thụ hưởng** | `Typography / Text` | `READONLY` | required | Không | Tên ngân hàng của hệ thống phòng gym (ví dụ: `MB Bank - Ngân hàng Quân Đội`) |
| **Số tài khoản thụ hưởng** | `Typography / Text + Copy Action` | `READONLY` | required | Không | Dãy số tài khoản ngân hàng chính thức của trung tâm; kèm nút thao tác `[ Sao chép ]` |
| **Chủ tài khoản** | `Typography / Text` | `READONLY` | required | Không | Tên pháp nhân: `CONG TY TNHH PARADISE GYM` |
| **Nội dung chuyển khoản** | `Typography / Code + Copy Action` | `READONLY` | required | `DYNAMIC`: Sinh theo mã đơn đăng ký | Mã cú pháp chuyển khoản duy nhất (ví dụ: `PGYMPAY 10425`); kèm nút thao tác `[ Sao chép ]` |
| **Nút thao tác [ Lưu mã QR ]** | `Button / Secondary` | `USER-INPUT` | optional | Không | Nút cho phép lưu ảnh mã VietQR về bộ nhớ điện thoại |
| **Nút thao tác [ Tôi đã chuyển khoản ]** | `Button / Primary CTA` | `USER-INPUT` | required | Không | Nút kiểm tra trạng thái giao dịch hoặc hoàn tất quay về màn hình Gói của tôi |
