...
6. SYS nạp và hiển thị danh sách các thẻ Card gói tập tương ứng bao gồm:
   ...
   - **Tiến độ sử dụng & Thanh Progress bar**:
    ...
    - **Nút thao tác trên thẻ**:
      ...
      - Nút **`[ Đóng băng ]`**: Hiển thị trên thẻ gói đang hoạt động và đã thanh toán để Hội viên chủ động đóng băng gói.
...
7. Khi Hội viên bấm nút **`[ Chi tiết gói ]`**, SYS mở modal **Chi tiết gói tập** bao gồm:
   ...
   - Nếu gói không đóng băng: Hiển thị nút **`[ Đóng băng gói ]`** (nếu gói đang hoạt động) và nút **`[ Gia hạn gói ]`** cho phép thanh toán gia hạn nhanh gói tập.
...
| **Nút [ Đóng băng ] trên thẻ** | `Action Button` | `USER-INPUT` | conditional | `CONDITIONAL`:<br>• **Hiện khi**: Gói đang hiệu lực, đã thanh toán, chưa bị đóng băng và thuộc sở hữu của hội viên<br>• **Ẩn khi**: Gói đang đóng băng, gói đã hủy/hết hạn, hoặc hội viên là thành viên nhóm được mời | Bấm mở modal Đóng băng gói tập |
...
| **Modal Chi tiết gói tập** | `Modal / Dialog` | `READONLY` | conditional | `CONDITIONAL`:<br>• **Hiện khi**: Bấm nút `[ Chi tiết gói ]`<br>• **Ẩn khi**: Bấm nút Đóng | Hiển thị: Thông tin gói, tiến độ, thành viên trong nhóm, thông báo bảo lưu (nếu đóng băng), nút thao tác tương ứng |
...
| **Nút [ Đóng băng gói ] trong modal** | `Action Button` | `USER-INPUT` | conditional | `CONDITIONAL`:<br>• **Hiện khi**: Gói đang hiệu lực, đã thanh toán, chưa đóng băng và thuộc sở hữu hội viên<br>• **Ẩn khi**: Gói đang đóng băng, đã hết hạn/hủy, hoặc là thành viên nhóm được mời | Bấm mở modal Đóng băng gói tập |
...
5. SYS gửi yêu cầu `POST /registrations/:id/freeze`, cập nhật trạng thái gói thành `❄️ Đang đóng băng`, lùi ngày kết thúc tương ứng số ngày đóng băng và ghi nhận lịch sử vào `package_freezes`.
