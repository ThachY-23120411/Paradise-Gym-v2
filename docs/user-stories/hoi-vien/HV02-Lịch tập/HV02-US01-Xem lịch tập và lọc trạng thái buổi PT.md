...
| :--- | :--- | :--- | :--- | :--- |
| **Lịch tháng (Calendar Picker)** | `USER-INPUT` | optional | `TRIGGER`: Chọn 1 ngày cụ thể sẽ kích hoạt lọc *Danh sách buổi tập* theo đúng ngày đó; click lại vào ngày đang chọn để bỏ chọn (deselect) sẽ kích hoạt hiển thị tất cả buổi tập | Widget lưới lịch tháng dạng bảng 7 cột (`T2`–`CN`), tiêu đề `< Tháng X Năm YYYY >` kèm 2 nút chuyển tháng `< >`. Ngày được chọn highlight vòng tròn màu xanh |
| **Bộ lọc Chip trạng thái** | `USER-INPUT` | required | `DYNAMIC`: Số lượng `(n)` trong ngoặc tự tính theo phạm vi ngày đang chọn (hoặc tất cả ngày nếu không chọn ngày) | Cho phép chọn 1 trong các chip: `Tất cả`, `Chờ xác nhận`, `Đã đặt`, `Đã hoàn thành`, `Đã hủy` |
| **Thẻ buổi tập (Session Card)** | `READONLY` | required | `DYNAMIC`: Danh sách các thẻ thay đổi theo phạm vi ngày được chọn ở trường TRIGGER *Lịch tháng* (hoặc tất cả các ngày nếu không chọn ngày) và theo *Bộ lọc Chip trạng thái* | Record booking của Hội viên, hiển thị đầy đủ: Khung giờ bắt đầu (ví dụ: `09:00`), Tên chi nhánh (ví dụ: `Quận 1`), Họ tên PT phụ trách (ví dụ: `Nguyễn Thành Long`), Tên gói tập (ví dụ: `PT 20 buổi`) và Badge trạng thái (`Chờ xác nhận`, `Đã đặt`, `Đang diễn ra`, `Đã hoàn thành`, `Đã hủy`) |
| **Nút `[ Xác nhận hoàn thành ]` trên Card** | `USER-INPUT` | conditional | `CONDITIONAL`: **Hiện khi** booking có trạng thái `Chờ xác nhận hoàn thành` (buổi tập đã kết thúc); **Ẩn khi** booking ở các trạng thái khác (`Đã đặt`, `Đã hủy`, `Đã hoàn thành`) | Nút CTA xanh viền/nền; bấm kích hoạt luồng xác nhận hoàn thành buổi tập (`HV02-US04`) |
| **Nút `[ Hủy lịch ]` trên Card** | `USER-INPUT` | conditional | `CONDITIONAL`: **Hiện khi** booking ở trạng thái `Đã đặt` (buổi tập chưa diễn ra); **Ẩn khi** booking ở trạng thái `Chờ xác nhận`, `Đang diễn ra`, `Đã hoàn thành`, `Đã hủy` | Nút phụ màu xám/đỏ nhạt; bấm mở Modal xác nhận hủy lịch (`HV02-US03`) |
