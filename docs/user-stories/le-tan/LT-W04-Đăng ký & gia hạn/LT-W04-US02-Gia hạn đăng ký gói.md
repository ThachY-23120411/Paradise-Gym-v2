...
7. Lễ tân chọn **Xác nhận lưu gia hạn**.
...
| Ngày bắt đầu mới | `Date Picker` | `USER-INPUT (AUTO-FILL)` | required | `TRIGGER`: Làm mốc tính toán Ngày kết thúc mới | SYS tự động tính toán mặc định theo 2 trường hợp:<br>• **Nếu gói cũ còn hạn**: Mặc định = `Ngày hết hạn cũ + 1 ngày`<br>• **Nếu gói cũ đã hết hạn**: Mặc định = `Ngày hiện tại + 1 ngày`<br>(Lễ tân có thể điều chỉnh tùy chọn qua ô chọn ngày) |
