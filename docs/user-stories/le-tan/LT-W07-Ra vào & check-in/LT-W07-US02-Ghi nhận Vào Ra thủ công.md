...
## Preconditions
- Lễ tân đã đăng nhập vào hệ thống Web quản lý, trong phạm vi chi nhánh làm việc (`branch scope`).
- Hệ thống đã có dữ liệu hồ sơ hội viên (`MEMBER_PROFILE`) và các gói đăng ký (`REGISTRATION`).

## Trigger
- Lễ tân nhập/quét thông tin hội viên tại Card **Kiểm soát ra/vào** trên màn hình W07 hoặc bấm nút **[⟲ Thủ công]** trên Card Thiết bị để mở modal **Ghi nhận ra/vào thủ công**.
- Màn hình liên quan: Web Lễ tân — W07 Ra / Vào, Card Kiểm soát ra/vào (cột trái) và modal Ghi nhận ra/vào thủ công.

## Main Flow

1. Lễ tân truy cập màn hình W07 Ra / Vào.
2. Lễ tân có thể lựa chọn 1 trong 2 hình thức ghi nhận thủ công:
   - **Hình thức 1 (Thao tác nhanh tại Card Kiểm soát ra/vào bên trái):**
     1. Khách đọc Số điện thoại (hoặc Họ tên / Mã HV); Lễ tân gõ SĐT/Mã HV/Họ tên (hoặc quét mã QR trên ứng dụng hội viên) tại ô tìm kiếm.
     2. SYS tự động hiển thị gợi ý danh sách hội viên khớp; Lễ tân chọn đúng tên hội viên.
     ...
     4. Lễ tân bấm nút CTA màu xanh lá (1 click).
   ...
   - **Hình thức 2 (Modal Ghi nhận ra/vào thủ công chuyên sâu qua nút [⟲ Thủ công]):**
     1. Lễ tân bấm nút **[⟲ Thủ công]** trên Card Thiết bị.
     2. SYS hiển thị modal Ghi nhận ra/vào thủ công.
     3. Lễ tân tìm kiếm và chọn Hội viên (`<Mã HV> - <Tên> - <SĐT>`).
     ...
     5. Lễ tân chọn Gói tập sử dụng (nếu hội viên chỉ có duy nhất 1 gói, SYS tự động chọn sẵn).
     6. Lễ tân chọn Loại sự kiện (`Vào` hoặc `Ra`), kiểm tra Điểm vào và Thời điểm ghi nhận.
     7. Lễ tân chọn Lý do thủ công (`Thiết bị lỗi`, `Không nhận diện được khuôn mặt`, `Khác`).
     8. Nếu chọn lý do `Khác`, Lễ tân nhập nội dung bắt buộc tại ô Mô tả lý do khác.
     9. Lễ tân bấm nút **Ghi nhận thủ công**.
3. SYS kiểm tra 6 điều kiện hợp lệ để vào tập (Profile ACTIVE, Gói còn hạn, Đã thanh toán 100%, Đúng chi nhánh, Còn số buổi, Trong giờ hoạt động):
   - Đủ 6 điều kiện: SYS ghi nhận bản ghi sự kiện ra/vào thủ công: `member_id`, `package_id`, `branch_id`, `direction` (`IN`/`OUT`), `source` = `MANUAL`, `reason`, `performed_by` (tài khoản Lễ tân), `timestamp`.
...
| :--- | :--- | :--- | :--- | :--- | :--- |
| Tìm kiếm / tra cứu hội viên | `Textbox (Search)` | `USER-INPUT` | required | `TRIGGER` | Lễ tân gõ SĐT, Họ tên hoặc Mã HV (hoặc quét mã QR) để tra cứu hội viên (`<Mã HV> · <Tên> · <SĐT>`); kích hoạt tự động nhận diện chiều và đổi nhãn nút CTA |
...
| Field / control | Loại UI Control | State | Required | Conditional / dynamic | Source / validation |
| :--- | :--- | :--- | :--- | :--- | :--- |
| Hội viên | `Select Dropdown (Searchable)` | `USER-INPUT` | required | `TRIGGER` | Lễ tân gõ SĐT, Mã HV hoặc Tên để tìm & chọn hội viên (`<Mã HV> - <Tên> - <SĐT>`); kích hoạt nạp danh sách gói tập của hội viên |
| Gói tập sử dụng | `Select Dropdown` | `USER-INPUT` | required | `DYNAMIC` | Nạp danh sách các gói đang ACTIVE và có hiệu lực tại chi nhánh của hội viên; Lễ tân chọn gói áp dụng cho lượt tập (tự chọn sẵn nếu hội viên chỉ có 1 gói) |
| Chi nhánh / điểm vào | `Readonly Text / Input` | `READONLY (PREFILL)` | optional | Không | SYS tự động nạp sẵn chi nhánh của Lễ tân và cổng mặc định (ví dụ: "Quận 1 · Gate-Q1-01") |
| Loại sự kiện | `Select Dropdown` | `USER-INPUT` | required | Không | Lễ tân chọn chiều sự kiện: `Vào` (Check-in) hoặc `Ra` (Check-out) |
...
| Lý do thủ công | `Select Dropdown` | `USER-INPUT` | required | `TRIGGER` | Lễ tân chọn lý do từ danh mục: `Thiết bị lỗi`, `Không nhận diện được khuôn mặt`, `Khác` |
| Mô tả lý do khác | `Textarea` | `USER-INPUT` | conditional | `CONDITIONAL`:<br>• **Hiện và bắt buộc khi**: `Lý do thủ công = Khác`<br>• **Ẩn khi**: `Lý do thủ công` chọn các giá trị còn lại | Lễ tân nhập diễn giải chi tiết lý do thao tác thủ công khi không thuộc các lý do định sẵn (tối đa 255 ký tự) |
...
- **Business rules / logic:**
  ...
  - Mỗi lượt ra/vào thủ công đều lưu rõ danh tính tài khoản Lễ tân thực hiện (`performed_by`), lý do và thời điểm thao tác thực tế.
...
**Trigger:** Lễ tân thực hiện ghi nhận vào/ra thủ công qua Card bên trái hoặc Modal [⟲ Thủ công].

```mermaid
flowchart TB
  subgraph B["Boundary — Web Lễ tân W07 / Ghi nhận thủ công"]
