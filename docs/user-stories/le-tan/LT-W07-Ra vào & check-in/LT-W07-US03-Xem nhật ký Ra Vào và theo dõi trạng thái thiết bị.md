...
- Màn hình liên quan: Web Lễ tân — W07 Ra / Vào, Card Thiết bị (cột trái) và Danh sách Nhật ký ra/vào hôm nay (cột phải).
...
2. SYS nạp và hiển thị **Card Thiết bị** ở cột bên trái:
   ...
   - Nút **[⟲ Thủ công]** (mở modal ghi nhận thủ công `LT-W07-US02`).
   - *(Lưu ý: Nút Cấu hình thiết bị bị ẩn đối với vai trò Lễ tân do thẩm quyền cấu hình thuộc menu W12 của QTV).*
...
5. Lễ tân xem danh sách sự kiện ra vào, theo dõi tình trạng thiết bị tại chi nhánh hoặc sử dụng **Bộ chọn ngày** trên thanh header để tra cứu lịch sử ra/vào các ngày trong quá khứ.
6. Khi Lễ tân chọn một ngày khác trên Bộ chọn ngày, SYS tự động truy vấn và cập nhật lại toàn bộ bảng sự kiện theo ngày được chọn.
...
| Nút Thủ công `[⟲ Thủ công]` | `Button (Secondary)` | `USER-INPUT` | optional | Không | Nút bấm mở modal Ghi nhận ra/vào thủ công (`LT-W07-US02`) |
...
- **Business rules / logic:**
  - Bảng nhật ký mặc định hiển thị toàn bộ các lượt ra/vào trong ngày hiện tại (`TODAY`) tại chi nhánh của Lễ tân.
  - Khi cần tra cứu đối soát lịch sử, Lễ tân chỉ cần chọn ngày tương ứng trên Bộ chọn ngày mà không cần rời màn hình.
  - Phản ánh cả các lượt vào/ra tự động qua Camera/Cổng thông minh lẫn các lượt ghi nhận thủ công do Lễ tân quầy thực hiện.
  - Card Thiết bị giúp Lễ tân nắm bắt tức thì tình trạng hoạt động của cổng và màn hình kiosk chào mừng K01 để kịp thời xử lý khi thiết bị offline.

## Activity Diagram — Swimlane
**Trigger:** Lễ tân mở màn hình W07 Ra / Vào để theo dõi sự kiện và trạng thái thiết bị.

```mermaid
flowchart TB
  subgraph B["Boundary — Web Lễ tân W07 / Giám sát Ra Vào & Thiết bị"]
    subgraph L0["Swimlane — Lễ tân"]
      I01(("Initial"))
      A01["Truy cập menu W07 Ra / Vào"]
      A02["Xem trạng thái thiết bị & Nhật ký sự kiện ngày hiện tại"]
      D01{"Lễ tân có đổi ngày xem nhật ký?"}
