...
## Preconditions
- Lễ tân đã đăng nhập hệ thống và có quyền tra cứu hồ sơ hội viên tại chi nhánh trực quầy.
- Lễ tân thao tác trong phạm vi chi nhánh được phân công (branch scope: cố định theo chi nhánh trực quầy).
...
1. Lễ tân mở menu **Hội viên & khách hàng** (W02).
2. SYS xác định branch scope của tài khoản Lễ tân theo chi nhánh đang làm việc.
3. SYS truy vấn và hiển thị bảng danh sách hội viên thuộc chi nhánh đó.
...
- **Business rules / logic:**
  ...
  - Dữ liệu trên bảng danh sách là chỉ đọc (`READONLY`); các thao tác thay đổi dùng các User Story thêm/sửa/đổi trạng thái tương ứng.
  - Sắp xếp mặc định theo thời gian tạo mới nhất hoặc theo mã hội viên.

## Exception Flows
- Không có dữ liệu phù hợp: SYS hiển thị empty state "Không tìm thấy hội viên nào trong chi nhánh".
...
flowchart TB
  subgraph B["Boundary — Web Lễ tân / W02 Danh sách hội viên"]
    ...
    subgraph L1["Swimlane — SYS"]
      S01["Xác định branch scope của Lễ tân"]
      S02["Truy vấn và hiển thị danh sách hội viên trong chi nhánh (Mã HV, Họ tên, SĐT, Email, Chi nhánh, Trạng thái hồ sơ)"]
      A01 --> S01 --> S02 --> F01
    end
  end
```
