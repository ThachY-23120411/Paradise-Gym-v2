...
- Hệ thống đã có các khoản hoa hồng PT được phát lệnh chi trả ở trạng thái `PAID` (đã hoàn tất) hoặc `PENDING_CONFIRMATION` (chờ PT xác nhận trên app), kèm phương thức `BANK_TRANSFER` hoặc `CASH`, mã chứng từ `payout_ref` (nếu có) và ngày giờ chi trả `paid_at`.
...
2. SYS tự động nạp danh sách toàn bộ các giao dịch hoa hồng đã phát lệnh chi trả (`status IN ('PAID', 'PENDING_CONFIRMATION')`) của các HLV trong phạm vi chi nhánh của QTV, sắp xếp theo thời gian chi trả mới nhất (`COALESCE(paid_at, created_at) DESC`).
3. SYS tổng hợp và hiển thị bộ 4 thẻ KPI thống kê lịch sử giải ngân:
   - `Tổng lượt chi trả`: Tổng số giao dịch hoa hồng trong danh sách (kèm chú thích số lượt đã chi và số lượt chờ PT xác nhận).
   - `Tổng tiền chi trả`: Tổng số tiền hoa hồng trong danh sách (VND).
   - `Chi qua VietQR / Ngân hàng`: Tổng số tiền và số giao dịch đã thanh toán qua VietQR.
   - `Chi tiền mặt tại quầy`: Tổng số tiền và số phiếu chi tiền mặt đã thanh toán trực tiếp tại quầy.
4. SYS hiển thị danh sách chi tiết các khoản chi trên bảng dữ liệu `dxDataGrid` (Kỳ hoa hồng, Họ tên & Mã HLV, Chi nhánh, Số buổi dạy, Doanh số quy đổi, Tỷ lệ %, Tiền hoa hồng, Hình thức chi trả, Trạng thái chi trả [Badge Đã chi trả / Chờ PT xác nhận], Mã GD / Phiếu chi, Thời gian chi trả, Người thực hiện chi, Nút Chi tiết).
5. QTV sử dụng các bộ lọc tác nghiệp:
   ...
   - Chọn lọc theo hình thức chi trả: *Tất cả hình thức*, *Chuyển khoản VietQR*, *Tiền mặt tại quầy*.
   - Chọn lọc theo trạng thái chi trả: *Tất cả trạng thái*, *Đã chi trả* (`PAID`), *Chờ PT xác nhận* (`PENDING_CONFIRMATION`).
6. SYS tự động lọc danh sách và cập nhật lại bộ 4 thẻ KPI tương ứng với kết quả lọc.
7. QTV bấm nút **[Chi tiết]** trên một dòng giao dịch: SYS mở popup hiển thị snapshot bất biến các buổi cấu thành kèm khối chứng từ chi trả (Thời gian phát lệnh, Hình thức, Mã giao dịch / Phiếu chi, Người duyệt chi, Trạng thái xác nhận của PT, Ghi chú).
8. QTV bấm nút **[Xuất file]**: SYS xuất toàn bộ dữ liệu lịch sử chi trả (gồm 13 cột thông tin kèm Trạng thái chi trả) ra file CSV/Excel để phục vụ lưu trữ sổ sách kế toán.
...
| Bộ lọc Trạng thái chi trả | `Select Dropdown (dxSelectBox)` | `USER-INPUT (PREFILL)` | required | `TRIGGER` | Tùy chọn: `ALL` (Tất cả trạng thái - mặc định), `PAID` (Đã chi trả), `PENDING_CONFIRMATION` (Chờ PT xác nhận) |
| Nút [Làm mới] | `Button (Outlined)` | `USER-INPUT` | optional | `Không` | Tải lại danh sách lịch sử chi trả và tính toán lại 4 thẻ KPI |
| Nút [Xuất file] | `Button (Outlined)` | `USER-INPUT` | optional | `Không` | Xuất danh sách lịch sử chi trả hiện tại ra file CSV kèm dấu phân cách chuẩn UTF-8 (13 cột dữ liệu) |
| Thẻ KPI Tổng lượt chi trả | `Metric Card (W().metrics)` | `READONLY` | required | `DYNAMIC` | Hiển thị tổng số giao dịch hoa hồng trong danh sách (`PAID` + `PENDING_CONFIRMATION`) theo bộ lọc hiện hành |
| Thẻ KPI Tổng tiền chi trả | `Metric Card (W().metrics)` | `READONLY` | required | `DYNAMIC` | Hiển thị tổng số tiền hoa hồng trong danh sách (VND) theo bộ lọc |
...
| Bảng dữ liệu Lịch sử chi trả | `DataGrid (dxDataGrid)` | `READONLY` | required | `DYNAMIC` | Bảng hiển thị các cột: Kỳ hoa hồng, Huấn luyện viên, Chi nhánh, Số buổi dạy, Doanh số quy đổi, Tỷ lệ %, Tiền hoa hồng (nổi bật tabular), Hình thức (Badge), Trạng thái chi trả (Badge success/info), Mã GD / Phiếu chi, Thời gian chi trả, Người chi trả, Nút Chi tiết |
...
flowchart TB
  subgraph B["Boundary — Web QTV W15 / Lịch sử chi trả hoa hồng"]
    ...
    subgraph L1["Swimlane — SYS"]
      S01["Truy vấn danh sách pt_commissions có status IN ('PAID', 'PENDING_CONFIRMATION')"]
