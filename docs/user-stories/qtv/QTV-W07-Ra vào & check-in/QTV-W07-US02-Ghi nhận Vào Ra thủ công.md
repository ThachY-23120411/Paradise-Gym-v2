...
## Preconditions
- QTV đã đăng nhập vào Web QTV, có quyền xử lý vận hành và mở menu **W07 · Ra / Vào**.
...
- Màn hình liên quan: Web QTV — W07 Ra / Vào, khu vực Ghi nhận thủ công (bên trái).

## Main Flow

1. QTV thao tác tại khu vực **Ghi nhận Vào/Ra thủ công** (phần bên trái màn hình W07).
2. QTV nhập SĐT hội viên vào ô `Tìm hội viên`.
3. SYS hiển thị danh sách hội viên gợi ý ➔ QTV chọn đúng hội viên (ví dụ: `HV001 - Nguyễn Văn A`).
...
5. QTV nhập **Lý do thao tác thủ công** (Trường **BẮT BUỘC** nhập giải trình để lưu audit trail kỹ lưỡng).
6. QTV bấm nút CTA (**Ghi nhận Vào** hoặc **Ghi nhận Ra**).
7. **SYS VẪN KIỂM TRA TOÀN BỘ 6 ĐIỀU KIỆN HỢP LỆ** (MEMBER_PROFILE active, gói còn hạn, thanh toán 100%, đúng chi nhánh, còn số buổi, trong giờ hoạt động). Tuyệt đối QTV/Lễ tân **không được dùng thủ công để bypass gói hết hạn hoặc chưa thanh toán**.
8. Nếu đủ điều kiện hợp lệ:
   - SYS ghi nhận bản ghi access event thủ công lưu vết audit: `member`, `branch`, `direction` (`IN` / `OUT`), `source` = `MANUAL`, `performed_by` (tài khoản QTV), `reason` (lý do bắt buộc), `timestamp`.
...
| Lý do thao tác thủ công | `USER-INPUT` | required | `DYNAMIC`: Bắt buộc nhập giải trình lý do thao tác thủ công để audit kỹ | QTV nhập |
...
**Trigger:** QTV nhập SĐT tìm hội viên và bấm nút Ghi nhận Vào / Ghi nhận Ra thủ công.

```mermaid
flowchart TB
  subgraph B["Boundary — Web QTV W07 / Form Ghi nhận thủ công bên trái"]
