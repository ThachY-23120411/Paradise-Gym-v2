...
## Preconditions
- QTV đăng nhập, có quyền thu tiền trong chi nhánh cụ thể được phân quyền (scope ALL không được ghi nhận mới).
...
## Main Flow
1. SYS mở modal Ghi nhận thanh toán; QTV tìm Hội viên bằng tên, mã hoặc SĐT.
2. SYS nạp các đăng ký chờ của đúng hội viên trong scope. QTV chọn đăng ký; nếu mở từ W04 thì prefill đúng đăng ký đó.
3. SYS hiển thị giá snapshot, voucher và số tiền phải trả 100% sau giảm giá; không cho nhập khoản thu thiếu.
4. QTV chọn CASH hoặc BANK_TRANSFER và nhập ghi chú tùy chọn. Với CASH, nhận đủ tiền thực tế rồi bấm xác nhận.
...
```mermaid
flowchart TB
  subgraph Boundary["Boundary - Web QTV / W08 Ghi nhận thanh toán"]
