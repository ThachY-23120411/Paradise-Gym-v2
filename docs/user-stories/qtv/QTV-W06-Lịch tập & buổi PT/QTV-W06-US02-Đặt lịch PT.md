...
## Preconditions
- QTV đã mở xem lịch PT của HLV trên màn hình W06, đã chọn HLV trên combobox và chọn ngày trên Date picker.
- Ngày được chọn không thuộc danh sách Ngày nghỉ lễ của phòng gym (`holidays`).
- Hội viên đã có gói PT hoặc Combo hợp lệ (được Lễ tân/QTV gán HLV này phụ trách, còn hạn và số buổi khả dụng > 0).

## Trigger
- QTV bấm nút **[+ Đặt lịch]** trên thanh công cụ hoặc click chuột vào ô giờ trống bất kỳ trên giao diện Calendar timeline (lưới 30 phút, từ 06:00 đến 22:00).
- Màn hình liên quan: Web QTV — W06 Lịch tập & buổi PT, Calendar Scheduler (dxScheduler) & modal **Đặt lịch PT**.

## Main Flow

1. QTV chọn HLV và ngày tác nghiệp trên bộ lọc.
2. QTV mở form đặt lịch bằng một trong 2 cách:
   - Cách 1: Bấm nút **[+ Đặt lịch PT]** trên thanh tác vụ.
...
4. QTV tra cứu và chọn **Hội viên** theo SĐT, Họ tên hoặc Mã hội viên.
5. SYS tự động truy vấn và đổ danh sách các gói PT hoặc Combo hợp lệ của hội viên mà HLV này đang phụ trách (`status = ACTIVE`, còn hạn và số buổi PT còn lại > 0).
6. QTV chọn **Gói PT sử dụng**.
7. Ngay khi chọn gói:
...
8. QTV lựa chọn một trong 2 phương thức hoàn tất:
   - **Phương thức 1 (Xác nhận trực tiếp trong modal):** QTV tinh chỉnh giờ bắt đầu trên dropdown (bước nhảy 15 phút), nhập Ghi chú buổi tập và bấm nút **[✓ Xác nhận đặt lịch]**.
   - **Phương thức 2 (Kéo chọn giờ trực quan trên Calendar):** QTV bấm nút **[📅 Kéo chọn giờ trên Calendar]**. Modal tự động đóng lại; trên Calendar xuất hiện **Thẻ đặt lịch dự kiến** với chiều cao tương ứng chính xác thời lượng gói vừa chọn (30p = 1 ô, 60p = 2 ô, 90p = 3 ô, 120p = 4 ô) kèm tên Hội viên và Tên gói. QTV nhấn giữ kéo thẻ lên/xuống hoặc click vào ô giờ trống để điều chỉnh giờ bắt đầu; thanh Floating Bar ở đáy màn hình hiển thị tóm tắt và nút **[✓ Xác nhận đặt lịch]**. QTV bấm nút này để chốt lịch.
9. SYS kiểm tra không bị xung đột giờ với các ca tập khác của HLV trong ngày; tạo bản ghi booking (`BOOKED`), gửi thông báo in-app cho HLV và Hội viên, xóa thẻ dự kiến và cập nhật timeline lịch tập.
...
| Hội viên | `Select Dropdown (Searchable)` | `USER-INPUT` | required | `TRIGGER`: Kích hoạt nạp danh sách gói PT hợp lệ | QTV gõ SĐT, Họ tên hoặc Mã để tra cứu trong `MEMBER_PROFILES` |
...
| Giờ bắt đầu | `Select Dropdown` | `USER-INPUT` | required | `TRIGGER`: Tính giờ kết thúc & kiểm tra xung đột | QTV chọn giờ bắt đầu linh động theo bước nhảy 15 phút (06:00 - 21:30) |
...
| Ghi chú cho buổi | `Textarea` | `USER-INPUT` | optional | `Không` | QTV nhập mục tiêu bài tập hoặc dặn dò thể lực hội viên |

## Alternate Flows

### AF-01 - Hủy Thao Tác
1. QTV chọn **Hủy** trên modal hoặc nút **[✕ Hủy chọn]** trên thanh Floating Bar.
...
1. Khi đang ở chế độ kéo thẻ trên Calendar, QTV bấm nút **[✏️ Đổi gói / thông tin]** trên Floating Bar.
2. SYS mở lại modal Đặt lịch PT với toàn bộ thông tin hội viên, gói và mốc giờ hiện tại để QTV hiệu chỉnh.
...
**Trigger:** QTV mở đặt lịch PT từ Calendar timeline hoặc danh sách ca trong menu W06.

```mermaid
flowchart TB
  subgraph B["Boundary — Web QTV W06 / Modal & Calendar Timeline"]
    subgraph L0["Swimlane — Quản trị viên (QTV)"]
      ...
      F01((("Final — Buổi tập PT được xếp lịch thành công")))
      F02((("Final — Báo lỗi xung đột lịch / Ngày lễ")))
    end
    ...
    subgraph L1["Swimlane — SYS"]
      ...
      S04 --> D01
      D01 -->|Hợp lệ| S05 --> F01
      D01 -->|Trùng lịch / Nghỉ lễ| S06 --> F02
    end
  end
```
